  ...
  public function ajaxMakeAComment($para) {
    ...
    if ($day_id > 0 && $content != "" && $username != "" && $email != "") {
      ...
      if ($re) {
        ...
        //      echo($view->getOneComment($last_com));
        $arr = ["day_id" => $day_id, "content" => $this->view->getOneCommentHTML($last_com)];
        echo(json_encode($arr, JSON_UNESCAPED_SLASHES));
      }
    }
    else {
      die("Please check inputs");
    }
  }
  ...
  public function ajaxMakeAReply($para) {
    ...
    if ($day_id > 0 && $content != "" && $username != "" && $email != "" && $reply_id > 0) {
      ...
      if ($re) {
        ...
        $arr      = ["com_id" => $com_id, "content" => $this->view->getOneCommentHTML($lastCom)];
        echo(json_encode($arr, JSON_UNESCAPED_SLASHES));
      }
    }
    else {
