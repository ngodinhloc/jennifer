...
class FileUploader {
  ...
  public function getRemovedFiles($customKey = 'file') {
    ...
    if (is_array($this->field['listInput']) && is_array($this->options['files'])) {
      foreach ($this->options['files'] as $key => $value) {
        if (!in_array($this->getFileAttribute($value, $customKey), $this->field['listInput']) &&
    ...
    if ($this->field['input']) {
      ...
      if ($validate === true) {
        ...
        for ($i = 0; $i < count($this->field['input']['name']); $i++) {
          ...
          $metas['old_title'] = substr($metas['old_name'], 0, (strlen($metas['extension']) >
                                                               0 ? -(strlen($metas['extension']) +
                                                                     1) : strlen($metas['old_name'])));
          ...
          $metas['title']     = substr($metas['name'], 0, (strlen($metas['extension']) >
                                                           0 ? -(strlen($metas['extension']) +
                                                                 1) : strlen($metas['name'])));
  ...
  private function validate($file = null) {
    if ($file == null) {
      ...
      if ($this->options['required'] && (isset($_SERVER) && strtolower($_SERVER['REQUEST_METHOD']) == "post") &&
          $this->field['count'] + count($this->options['files']) == 0
      ) {
        return $this->codeToMessage('required_and_no_file');
      }
      if (($this->options['limit'] &&
           $this->field['count'] + count($this->options['files']) > $this->options['limit']) ||
          ($ini[3] != 0 && ($this->field['count']) > $ini[3])
      ) {
      ...
      if ($this->options['extensions'] && (!in_array($file['extension'], $this->options['extensions']) &&
                                           !in_array($file['type'], $this->options['extensions']))
      ) {
  ...
  private function generateFilename($conf, $file, $skip_replace_check = false) {
    ...
    if (!$this->options['replace'] && !$skip_replace_check) {
      ...
      while (file_exists($this->options['uploadDir'] . $string)) {
        $file['title'] = $title . " ({$i})";
        $conf[0]       = $type == "auto" || $type == "name" || strpos($string, "{random}") !== false ? $type
          : $type . " ({$i})";
