...
class HTML2PDF {
  ...
  protected $_hideHeader = [];     // array : list of pages which the header gonna be hidden
   ...
   */
  public function __construct($orientation = 'P', $format = 'A4', $langue = 'fr', $unicode = true, $encoding = 'UTF-8',
                              $marges = [5, 5, 5, 8]) {
   ...
   * @return null
   */
  public function createIndex($titre = 'Index', $sizeTitle = 20, $sizeBookmark = 15, $bookmarkTitle = true,
                              $displayPage = true, $onPage = null, $fontName = 'helvetica') {
  ...
  protected function _vueHTML($content) {
    $content = preg_replace('/<page_header([^>]*)>/isU', '<hr>' . HTML2PDF_locale::get('vue01') .
                                                         ' : $1<hr><div$1>', $content);
    $content = preg_replace('/<page_footer([^>]*)>/isU', '<hr>' . HTML2PDF_locale::get('vue02') .
                                                         ' : $1<hr><div$1>', $content);
    $content = preg_replace('/<page([^>]*)>/isU', '<hr>' . HTML2PDF_locale::get('vue03') .
                                                  ' : $1<hr><div$1>', $content);
...
<html>
  ...
  protected function _setMargins() {
    ...
    $this->_margeBottom = $this->_defaultBottom +
                          (isset($this->_background['bottom']) ? $this->_background['bottom'] : 0);
    ...
    if ($this->_isInParagraph !== false) {
      $this->_pageMarges[floor($this->_margeTop * 100)] = [$this->_isInParagraph[0],
                                                           $this->pdf->getW() - $this->_isInParagraph[1]];
    }
    else {
      $this->_pageMarges[floor($this->_margeTop * 100)] = [$this->_margeLeft, $this->pdf->getW() - $this->_margeRight];
    }
  }
  ...
  protected function _DEBUG_add($name, $level = null) {
    ...
    $name  = str_repeat('  ', $this->_debugLevel) . $name .
  ...
  protected function _saveMargin($ml, $mt, $mr) {
    // save old margins
    $this->_marges[] = ['l' => $this->pdf->getlMargin(), 't' => $this->pdf->gettMargin(),
                        'r' => $this->pdf->getrMargin(), 'page' => $this->_pageMarges];
  ...
  protected function _listeAddLevel($type = 'ul', $style = '', $img = null) {
    ...
    if (!in_array($style, ['lower-alpha', 'upper-alpha', 'upper-roman', 'lower-roman', 'decimal', 'square', 'circle',
                           'disc', 'none'])
    ) {
      $style = '';
    }
  ...
  protected function _makeHTMLcode() {
    ...
    for ($this->_parsePos = 0; $this->_parsePos < count($this->parsingHtml->code); $this->_parsePos++) {
      ...
      if (in_array($action['name'], ['table', 'ul', 'ol']) && !$action['close']) {
        ...
        while (isset($this->parsingHtml->code[$this->_tempPos]) &&
               !($this->parsingHtml->code[$this->_tempPos]['name'] == $tagOpen &&
                 $this->parsingHtml->code[$this->_tempPos]['close'])) {
          // make the action
  ...
  protected function _makeBreakLine($h, $curr = null) {
    if ($h) {
      if (($this->pdf->getY() + $h < $this->pdf->getH() - $this->pdf->getbMargin()) || $this->_isInOverflow ||
          $this->_isInFooter
      ) {
  ...
  protected function _drawImage($src, $subLi = false) {
    ...
    if ($parentWidth > $w && $float != 'left') {
      if ($float == 'right' || $this->parsingCss->value['text-align'] == 'li_right') {
        $x = $parentX + $parentWidth - $w - $this->parsingCss->value['margin']['r'] -
             $this->parsingCss->value['margin']['l'];
      }
    }
      ...
      if (isset($param['backimg'])) {
        ...
        if ($background['img']) {
          ...
          if (count($infos) > 1) {
            ...
            $background['posX']  = $this->parsingCss->ConvertToMM($background['posX'], $this->pdf->getW() -
                                                                                       $imageWidth);
            $background['posY']  = $this->parsingCss->ConvertToMM($background['posY'], $this->pdf->getH() -
                                                                                       $imageHeight);
          }
          else {
            $background = [];
  ...
  protected function _tag_open_DIV($param, $other = 'div') {
    ...
    if (!$this->parsingCss->value['position']) {
      ...
      $old         = $this->parsingCss->getOldValues();
      $parentWidth = $old['width'] ? $old['width']
        : $this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin();
  ...
  protected function _tag_open_FIELDSET($param) {
    ...
    for ($tempPos = $this->_parsePos + 1; $tempPos < count($this->parsingHtml->code); $tempPos++) {
      ...
      if ($action['name'] == 'legend' && !$action['close']) {
        ...
        $this->parsingHtml->code[$legendOpenPos]['param']['moveLeft'] = 2 -
                                                                        $this->parsingCss->value['border']['l']['width'] -
                                                                        $this->parsingCss->value['padding']['l'];
  ...
  protected function _tag_open_WRITE($param) {
    $fill = ($this->parsingCss->value['background']['color'] !== null &&
             $this->parsingCss->value['background']['image'] === null);
    if (in_array($this->parsingCss->value['id_tag'], ['fieldset', 'legend', 'div', 'table', 'tr', 'td', 'th'])) {
      $fill = false;
    }
    ...
    if (count($words)) {
      ...
      $this->pdf->setXY($this->pdf->getX(), $y + $dh + $dy);
      $this->pdf->Cell(($align ==
                        'L' ? $w : $this->parsingCss->value['width']), $h, $txt, 0, 0, $align, $fill, $this->_isInLink);
      $this->pdf->setXY($this->pdf->getX(), $y);
      $this->_maxH = max($this->_maxH, $lh);
  ...
  protected function _tag_open_H1($param, $other = 'h1') {
    ...
    $size                                   = ['h1' => '28px', 'h2' => '24px', 'h3' => '20px', 'h4' => '16px',
                                               'h5' => '12px', 'h6' => '9px'];
  ...
  protected function _tag_open_P($param) {
    ...
    $this->pdf->setXY($this->pdf->getX() - $this->parsingCss->value['margin']['l'], $this->pdf->getY() -
                                                                                    $this->parsingCss->value['margin']['t']);
    ...
    if ($this->parsingCss->value['text-indent'] > 0) {
      $y                                  = $this->pdf->getY() + $this->parsingCss->value['margin']['t'] +
                                            $this->parsingCss->value['padding']['t'];
      $this->_pageMarges[floor($y * 100)] = [$mL + $this->parsingCss->value['text-indent'], $this->pdf->getW() - $mR];
  ...
  protected function _tag_open_TABLE($param, $other = 'table') {
    ...
    if ($this->_subPart) {
      ...
      HTML2PDF::$_tables[$param['num']]['marge']['t']      = $this->parsingCss->value['padding']['t'] +
                                                             $this->parsingCss->value['border']['t']['width'] +
                                                             HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
      HTML2PDF::$_tables[$param['num']]['marge']['r']      = $this->parsingCss->value['padding']['r'] +
                                                             $this->parsingCss->value['border']['r']['width'] +
                                                             HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
      HTML2PDF::$_tables[$param['num']]['marge']['b']      = $this->parsingCss->value['padding']['b'] +
                                                             $this->parsingCss->value['border']['b']['width'] +
                                                             HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
      HTML2PDF::$_tables[$param['num']]['marge']['l']      = $this->parsingCss->value['padding']['l'] +
                                                             $this->parsingCss->value['border']['l']['width'] +
                                                             HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5;
      ...
      $this->parsingCss->value['width'] -= HTML2PDF::$_tables[$param['num']]['marge']['l'] +
                                           HTML2PDF::$_tables[$param['num']]['marge']['r'];
    }
    else {
      ...
      HTML2PDF::$_tables[$param['num']]['td_x']    = HTML2PDF::$_tables[$param['num']]['marge']['l'] +
                                                     HTML2PDF::$_tables[$param['num']]['curr_x'];
      HTML2PDF::$_tables[$param['num']]['td_y']    = HTML2PDF::$_tables[$param['num']]['marge']['t'] +
  ...
  protected function _tag_close_TABLE($param) {
    ...
    if ($this->_subPart) {
      ...
      HTML2PDF::$_tables[$param['num']]['width'] = HTML2PDF::$_tables[$param['num']]['marge']['l'] +
                                                   HTML2PDF::$_tables[$param['num']]['marge']['r'];
      ...
      $h0 += HTML2PDF::$_tables[$param['num']]['thead']['height'] +
             HTML2PDF::$_tables[$param['num']]['tfoot']['height'];
      ...
      if (count(HTML2PDF::$_tables[$param['num']]['height']) > 1) {
        $y = $this->_margeTop +
             HTML2PDF::$_tables[$param['num']]['height'][count(HTML2PDF::$_tables[$param['num']]['height']) - 1];
      }
  ...
  protected function _tag_open_TR($param, $other = 'tr') {
    ...
    if (!$this->_subPart) {
      ...
      $ty = null;
      for ($ii = 0; $ii <
                    count(HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                                     1]); $ii++) {
        $ty = max($ty, HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                                  1][$ii]['h']);
      }
      ...
      if (!$this->_isInTfoot &&
          HTML2PDF::$_tables[$param['num']]['td_y'] + HTML2PDF::$_tables[$param['num']]['marge']['b'] + $ty + $hfoot >
          $this->pdf->getH() - $this->pdf->getbMargin()
      ) {
        ...
        HTML2PDF::$_tables[$param['num']]['curr_y'] = $this->pdf->getY();
        HTML2PDF::$_tables[$param['num']]['td_y']   = HTML2PDF::$_tables[$param['num']]['curr_y'] +
                                                      HTML2PDF::$_tables[$param['num']]['marge']['t'];
  ...
  protected function _tag_close_TR($param) {
    ...
    if (!$this->_subPart) {
      ...
      for ($ii = 0; $ii <
                    count(HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                                     1]); $ii++) {
        if (HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                       1][$ii]['rowspan'] == 1
        ) {
      ...
      // new position
      HTML2PDF::$_tables[$param['num']]['td_x'] = HTML2PDF::$_tables[$param['num']]['curr_x'] +
                                                  HTML2PDF::$_tables[$param['num']]['marge']['l'];
  ...
  protected function _tag_open_TD($param, $other = 'td') {
        ...
        if (count($colParam['style']['width'])) {
          ...
          foreach ($colParam['style']['width'] as $width) {
            if (substr($total, -1) == '%' && substr($width, -1) == '%') {
              ...
              $total = ($this->parsingCss->ConvertToMM($total, $last) + $this->parsingCss->ConvertToMM($width, $last)) .
                       'mm';
            }
          }
        }
    ...
    $marge      = [];
    $marge['t'] = $this->parsingCss->value['padding']['t'] + 0.5 * HTML2PDF::$_tables[$param['num']]['cellspacing'] +
                  $this->parsingCss->value['border']['t']['width'];
    $marge['r'] = $this->parsingCss->value['padding']['r'] + 0.5 * HTML2PDF::$_tables[$param['num']]['cellspacing'] +
                  $this->parsingCss->value['border']['r']['width'];
    $marge['b'] = $this->parsingCss->value['padding']['b'] + 0.5 * HTML2PDF::$_tables[$param['num']]['cellspacing'] +
                  $this->parsingCss->value['border']['b']['width'];
    $marge['l'] = $this->parsingCss->value['padding']['l'] + 0.5 * HTML2PDF::$_tables[$param['num']]['cellspacing'] +
                  $this->parsingCss->value['border']['l']['width'];
    ...
    if ($this->_subPart) {
      ...
      for ($j = 0; $j < $rowspan; $j++) {
        for ($i = 0; $i < $colspan; $i++) {
          ...
          [HTML2PDF::$_tables[$param['num']]['corr_x'] + $i] = ($i + $j > 0) ? '' : [$x, $y, $colspan, $rowspan];
        }
      }
      ...
      $this->parsingCss->value['width'] = HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'] - $marge['l'] -
                                          $marge['r'];
  ...
  protected function _tag_close_TD($param) {
    ...
    if ($this->_subPart) {
      ...
      HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                 1][HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['w'] = max([$w0,
                                                                                                                  $w2]);
      HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                 1][HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['h'] = max([$h0,
                                                                                                                  $h2]);

      // real position of the content
      HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                 1][HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['real_w'] = $w0;
      HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                 1][HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['real_h'] = $h0;
      ...
      HTML2PDF::$_tables[$param['num']]['td_x'] += HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] -
                                                                                              1][HTML2PDF::$_tables[$param['num']]['td_curr'] -
                                                                                                 1]['w'];
    }
  ...
  protected function _tag_open_IMG($param) {
    ...
    $this->parsingCss->value['background'] = ['color' => null, 'image' => null, 'position' => null, 'repeat' => null];
  ...
  protected function _tag_open_INPUT($param) {
    ...
    switch($param['type']) {
      case 'checkbox':
        ...
        $this->pdf->CheckBox($name, $w, $checked, $prop, [], ($param['value'] ? $param['value'] : 'Yes'), $x, $y);
        break;
      ...
      case 'radio':
        ...
        $checked = (isset($param['checked']) && $param['checked'] == 'checked');
        $this->pdf->RadioButton($name, $w, $prop, [], ($param['value'] ? $param['value'] : 'On'), $checked, $x, $y);
        break;
