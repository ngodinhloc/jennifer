...
class HTML2PDF_parsingCss {
  ...
  protected $_htmlColor = []; // list of the HTML colors
  ...
  public function analyse($tagName, &$param, $legacy = null) {
    ...
    if ($noWidth && in_array($tagName, ['div', 'blockquote', 'fieldset']) && $this->value['position'] != 'absolute') {
  ...
  public function readBorder($css) {
    ...
    foreach ($css as $value) {
      ...
      else if (in_array($value, ['solid', 'dotted', 'dashed', 'double'])) {
        $type = $value;
