  ...
  class User extends Model {
    ...
    public function getRightRelatedDayHTML($day, $month, $year, $location) {
      ...
      if ($result) {
        foreach ($result as $row) {
          if (isset($row['id'])) {
            $link = Common::getDayLink($row);
            $photos = trim($row['photos']);
            $firstPhoto = "";
            if ($photos != "") {
              $photos = explode(',', $photos);
              $photo = $photos[0];
              $photoUrl = Common::getPhotoURL($photo, PHOTO_THUMB_NAME);
    ...
    public function getDays($from, $order) {
      $limit = ($from == 0) ? (NUM_PER_PAGE * 2) : NUM_PER_PAGE;
      $orderCond = null;
      switch ($order) {
        case ORDER_BY_ID:
          $orderCond = ["id" => "DESC"];
    ...
    protected function getOneDayHTML($row) {
      ...
      if ($photos != "") {
        $photos = explode(',', $photos);
        $fistPhoto = $photos[0];
        $photoUrl = Common::getPhotoURL($fistPhoto, PHOTO_TITLE_NAME);
      ...
      if (strlen($preview) > PREVIEW_LENGTH) {
        $preview = Common::subString($preview, PREVIEW_LENGTH, 3);
