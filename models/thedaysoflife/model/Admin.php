<?php
  ...
  class Admin extends Model {
    ...
    public function getPhotoByYearMonth($year, $month) {
      ...
      foreach ($result as $row) {
        ...
        foreach ($photos as $i => $name) {
          $photo_full = Com::getPhotoName($name, Configs::PHOTO_FULL_NAME);
          $photo_title = Com::getPhotoName($name, Configs::PHOTO_TITLE_NAME);
          $photo_thumb = Com::getPhotoName($name, Configs::PHOTO_THUMB_NAME);
    ...
    public function updateDay($day) {
      ...
      $day["content"] = $this->escapeString($day["content"], true);
    ...
    public function getDayList($page) {
      $limit = Configs::NUM_PER_PAGE_ADMIN;
      $from = $limit * ($page - 1);
